...
let siemMonkeyInUI = setTimeout(function() {
    ...
    }, 5000) //TODO: иногда не успевает, надо придумать способ получше
            ...
            if (addedNode instanceof Node && (addedNode.className === "mc-dl-conditional ng-scope")) {
                if (addedNode.children.length = 2 && addedNode.children[0].innerHTML === "uuid") {
                    if ('options' in options &&
                        ...
                        options.options.dont_show_save_event_icons == true) {; //если задана опция "Не показывать кнопки сохранения JSON события", то и не показываем
                    } else {
...
function ProcessHandler(addedNode) {
    ...
    ancestors_branch_icon.click(function() {
        ...
        let object_process_guid = getFieldValueFromSidebar("uuid");
        ...
        if (msgid === 'execve' || msgid === '1' || msgid === '4688') {
    ...
    session_tree_icon.click(function() {
        var commandline = getFieldValueFromSidebar("object.process.cmdline");
        let object_process_uuid = getFieldValueFromSidebar("uuid");
        ...
        let session = getFieldValueFromSidebar("subject.account.session_id");
        if (session === null) {
            session = getFieldValueFromSidebar("object.account.session_id");
        }
        let time = getTimeValueFromSidebar();
        ...
        if (msgid === '4688') {
            ...
            getdata(siemUrl, `event_src.host = "${event_src_host}" and msgid = "${processStartMsgid}" and subject.account.session_id = ${session} and (correlation_name = null)`, count, processTree, "", ttimeto - 86400, ttimeto);
        }
    });
