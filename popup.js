...
async function onPageDetailsReceived(details) {
    ...
    if (typeof selectedEventTime === 'undefined') {
        ...
        selectedEventTimeParsedTo = selectedEventTimeParsed.clone().add(30, 'm');
        selectedEventTimeParsedFrom = selectedEventTimeParsed.clone().subtract(30, 'm');
        $("#datepickerTo").datepicker().datepicker("setDate", selectedEventTimeParsedTo.toDate());
        $("#datepickerFrom").datepicker().datepicker("setDate", selectedEventTimeParsedFrom.toDate());
    }
    ...
    $(`<div class="monkey">Показать сессии и число процессов в них на узле </div>`)
        ...
        .click(function() {
            ...
            let filter = `event_src.host = "${event_src_host}" and (msgid = "4688" or msgid = "1" or msgid = "execve") ` +
                `and (correlation_name = null)`;
            if (details.params['object.account.session.id']) {
                countGroupBy(siemUrl, filter, "object.account.session_id", processTree);
            } else {
                countGroupBy(siemUrl, filter, "subject.account.session_id", processTree);
            }
    ...
    let fl = true;
    let session = details.params['subject.account.session_id'].replace(/[\n\r'"]+/g, '');
    if (session === null) {
        fl = false
        session = details.params['object.account.session_id'].replace(/[\n\r'"]+/g, '');
    }
    let process_name = details.params["object.process.name"]
    if (!process_name) {
        process_name = details.params["subject.process.name"]
    }

    $(`<div class="monkey">Найти процессы в сессии (status != 'failure') </div>`)
        .appendTo("#mlinks")
        .click(function() {
            let count = $("#count").val();
            $('#output').replaceWith("<div id=\"output\"></div>");
            let filter
            if (fl) {
                filter = `event_src.host = "${event_src_host}" and (msgid = "4688" or msgid = "1" or msgid = "execve") ` +
                    `and subject.account.session_id = ${session} and (correlation_name = null) and status != 'failure'`
            } else {
                filter = `event_src.host = "${event_src_host}" and (msgid = "4688" or msgid = "1" or msgid = "execve") ` +
                    `and object.account.session_id = ${session} and (correlation_name = null) and status != 'failure'`
            };
    ...
    $(`<div class="monkey">Родители процесса  </div>`)
        ...
        .append($('<span>', { class: 'parameter' }).text(`${process_name.trim("↵")}`))

    CommandlineText = details.params[commandlineField]
    if (!CommandlineText) {
        CommandlineText = details.params["subject.process.meta"]
    }
    $(`<div class="monkey">Все потомки процесса  </div>`)
        ...
        .append($('<span>', { class: 'parameter' }).text(`${process_name.trim("↵")}`))
        .append($('<span>', { class: 'parameter' }).text(``))
    $(`<div class="monkey">CommandLine:  </div>`)
        .appendTo("#mlinks")
